JQTWEETT = {
	...
	loadTweets: function() {
		$.ajax({
			...
			dataType: 'jsonp',
			data: {
				screen_name: JQTWEETT.user,
				include_rts: true,
				count: JQTWEETT.numTweets,
			...
			success: function(data, textStatus, xhr) {
				 ...
				 for (var i = 0; i < data.length; i++) {
					...
					$(JQTWEETT.appendTo).append(
						html.replace('IMG_TAG', img)
							.replace('TWEET_TEXT', JQTWEETT.ify.clean(data[i].text, img) )
							.replace(/USER/g, data[i].user.screen_name)
							.replace('AGO', JQTWEETT.timeAgo(data[i].created_at) )
